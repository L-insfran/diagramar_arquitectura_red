...
export const createDeviceTemplateValidator = vine.compile(
  vine.object({
    ...
    rackUnits: vine.number().min(1).max(100).optional(),
    imageUrl: vine.string().trim().maxLength(500).optional(),
    frontViewUrl: vine.string().trim().maxLength(500).optional(),
    rearViewUrl: vine.string().trim().maxLength(500).optional(),
    powerConsumptionW: vine.number().min(0).optional(),
    weightKg: vine.number().min(0).optional(),
...
export const createDeviceTemplatePortValidator = vine.compile(
  vine.object({
    ...
    speed: vine.string().trim().maxLength(50).optional(),
